        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 bg-slate-900/60 backdrop-blur-sm animate-fadeIn">
          ...
          >
            ...
            <div className="p-6 space-y-4">
              ...
              {/* Previously Used Google Accounts List */}
              {savedGoogleAccounts.length > 0 && !isAddingNewGoogleAccount ? (
                <div className="space-y-2">
                  <p className="text-xs font-bold text-slate-600 uppercase tracking-wider">
                    {activeLanguage === 'ta' ? 'கணக்கைத் தேர்ந்தெடுக்கவும்' : 'Choose an account'}
                  </p>
                  <div className="space-y-1.5 max-h-56 overflow-y-auto">
                    {savedGoogleAccounts.map((account, idx) => (
                      <div
                        key={idx}
                        onClick={() => handleCompleteGoogleLogin(account.email, account.name)}
                        className="group p-2.5 rounded-2xl border border-slate-200/90 hover:border-sky-300 hover:bg-sky-50/60 flex items-center justify-between transition-all cursor-pointer shadow-2xs"
                      >
                        <div className="flex items-center space-x-3 min-w-0">
                          <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-sky-600 via-blue-500 to-indigo-600 text-white flex items-center justify-center font-black text-xs shadow-xs flex-shrink-0">
                        ...
                        <div className="flex items-center space-x-1.5 flex-shrink-0">
                          ...
                          <ChevronRight className="w-4 h-4 text-slate-300 group-hover:text-sky-600 group-hover:translate-x-0.5 transition-all" />
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <button
                    ...
                    className="w-full mt-2 py-2.5 px-3 rounded-2xl border border-dashed border-slate-300 hover:border-sky-400 hover:bg-sky-50 text-sky-700 text-xs font-bold flex items-center justify-center space-x-2 transition-all cursor-pointer"
                  >
                    <Plus className="w-4 h-4" />
                    <span>
                      {activeLanguage === 'ta' ? 'வேறு Google கணக்கை பயன்படுத்தவும்' : 'Use another Google account'}
                    </span>
                  </button>
                </div>
              ) : (
