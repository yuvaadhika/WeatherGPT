...
export default function InstallAppModal({ isOpen, onClose, activeLanguage = 'en' }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/70 backdrop-blur-md animate-fadeIn">
      <div className="relative w-full max-w-lg bg-gradient-to-b from-slate-900 via-slate-900 to-slate-950 border border-sky-500/30 rounded-3xl shadow-2xl overflow-hidden text-white animate-scaleUp">
        {/* Glow Header Accent */}
        <div className="absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-sky-400 via-cyan-400 to-indigo-500"></div>

        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-2 rounded-2xl bg-slate-800/80 hover:bg-slate-700 text-slate-400 hover:text-white transition-all cursor-pointer z-10"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="p-6 sm:p-7 space-y-5 max-h-[85vh] overflow-y-auto">
          ...
          <div className="flex items-center space-x-3.5">
            <div className="relative">
              <img
                src="/wyndra-logo.png"
                alt="WeatherGPT App"
                className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl shadow-lg border border-sky-400/40 p-1 bg-slate-800/80 object-contain"
              />
            ...
            <div>
              <div className="flex items-center space-x-2">
                <h3 className="text-xl sm:text-2xl font-black tracking-tight text-white">WeatherGPT</h3>
                <span className="px-2 py-0.5 rounded-full bg-sky-500/20 text-sky-300 border border-sky-400/40 text-[10px] font-extrabold uppercase">
                  PWA App
                </span>
              </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
            <div className="p-3 rounded-2xl bg-slate-800/60 border border-sky-500/20 space-y-1">
            ...
            <div className="p-3 rounded-2xl bg-slate-800/60 border border-emerald-500/20 space-y-1">
            ...
            <div className="p-3 rounded-2xl bg-slate-800/60 border border-indigo-500/20 space-y-1">
          ...
          {installSuccess || pwaState.isInstalled ? (
            <div className="p-4 rounded-2xl bg-emerald-950/80 border border-emerald-500/40 text-emerald-200 flex items-center space-x-3">
            ...
            <div className="space-y-4">
              ...
              {pwaState.canInstall && (
                <button
                  ...
                  className="w-full py-3.5 px-5 rounded-2xl bg-gradient-to-r from-sky-500 via-cyan-500 to-blue-600 hover:from-sky-400 hover:to-blue-500 text-white font-black text-sm sm:text-base flex items-center justify-center space-x-2 shadow-lg shadow-sky-500/25 transition-all transform active:scale-98 cursor-pointer disabled:opacity-50"
                >
              ...
              <div className="p-4 rounded-2xl bg-slate-800/70 border border-slate-700/60 space-y-2.5">
                <div className="flex items-center space-x-2 text-xs font-bold text-sky-400">
                  {platform === 'ios' ? <Smartphone className="w-4 h-4" /> : platform === 'windows' || platform === 'mac' ? <Monitor className="w-4 h-4" /> : <Smartphone className="w-4 h-4" />}
                  <span>
                    {platform === 'ios'
                ...
                {platform === 'ios' ? (
                  <ol className="text-xs text-slate-300 space-y-2 pl-1 list-none">
                    <li className="flex items-center space-x-2">
                      <span className="w-5 h-5 rounded-full bg-sky-500/20 text-sky-300 font-bold flex items-center justify-center text-[11px] flex-shrink-0">1</span>
                      <span>
                        Safari பிரவுசரின் கீழே உள்ள <Share2 className="w-3.5 h-3.5 inline text-sky-400 mx-1" /> <strong>Share</strong> ஐகானை தட்டவும்.
                      </span>
                    </li>
                    <li className="flex items-center space-x-2">
                      <span className="w-5 h-5 rounded-full bg-sky-500/20 text-sky-300 font-bold flex items-center justify-center text-[11px] flex-shrink-0">2</span>
                      <span>
                        கீழே ஸ்க்ரோல் செய்து <PlusSquare className="w-3.5 h-3.5 inline text-sky-400 mx-1" /> <strong>'Add to Home Screen'</strong> என்பதை தேர்ந்தெடுக்கவும்.
                      </span>
                    </li>
                    <li className="flex items-center space-x-2">
                      <span className="w-5 h-5 rounded-full bg-sky-500/20 text-sky-300 font-bold flex items-center justify-center text-[11px] flex-shrink-0">3</span>
